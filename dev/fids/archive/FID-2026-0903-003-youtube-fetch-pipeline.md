...
- Live fetch leg (2026-09-03, real YouTube Data API + production Firestore): **PASS 6/6** via `scripts/live-fetch-verify.ts` — real channel (@TwoMinutePapers) fetched 50 videos with zero warnings; ratings in [0.008, 0.413] ⊂ [0,1]; dedupe idempotent (doc-id set identical across two runs); resolution cache persisted (UCbfYPyITQ-7l4upoX8nvctg); source metadata success recorded (lastError null, totalFetched 400). Duplicate-source hazard found during verification and documented in the script: deterministic content doc ids make duplicate sources for the same channel converge on one doc — verifier reuses an existing source by URL and never deletes operator data.
- Status `verified` (webhook + service cycle + live-API leg); `closed` pending git commit hash (G2).
